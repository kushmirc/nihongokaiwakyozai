...
//    }
	...
	*/
    ...
    protected function _replace_images($tag, $attr, $text, $url, $q = '', &$noresize = false) {
        ...
        if($ext === 'css' || $ext === 'js') { //no need to calculate the image size if it's CSS
        ...
        if($noresize) {
            $extra_by_ext = ($ext == 'css' || $ext == 'js' ? '+v_' . $this->ctrl->cssCacheVer . ($this->ctrl->options->settings_compression_webp && $ext == 'css' ? '+to_webp' : '') : '');
            $inlinePlaceholder = $this->ctrl->get_api_url(false, false, $this->ctrl->get_extension( $url )) . $extra_by_ext . '/' . ShortPixelUrlTools::absoluteUrl($url);

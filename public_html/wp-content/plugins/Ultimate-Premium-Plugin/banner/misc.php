          ...
          if (!is_wp_error($installed) && $installed) {
            ...
            if (is_null($activate)) {
              ...
              if ($_POST['slug'] === 'cdp') {
                ...
                $url = admin_url('admin.php?page=copy-delete-posts', 'admin');
              }

              // Redirection for MPU
              if ($_POST['slug'] === 'mpu') {
                update_option('wp_mypopups_do_activation_redirect', true);
                $url = admin_url('admin.php?page=wp-mypopups', 'admin');
              }

              // Redirection for USM
              if ($_POST['slug'] === 'usm') {
                update_option('sfsi_plugin_do_activation_redirect', true);
                $url = admin_url('admin.php?page=sfsi-options', 'admin');
              }

              // Redirection for BMI
              if ($_POST['slug'] === 'bmi') {
                update_option('_bmi_redirect', true);
                $url = admin_url('admin.php?page=backup-migration', 'admin');
              }

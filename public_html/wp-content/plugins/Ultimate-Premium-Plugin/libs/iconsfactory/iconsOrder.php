...
if(!function_exists('sfsi_premium_admin_default_icons_order')){
    ...
    function sfsi_premium_admin_default_icons_order($option1=false,$isDesktop=true){
        ...
        array("iconName" => "fbmessenger" , "index"  => 21),
        array("iconName" => "mix"         , "index"  => 22),
        array("iconName" => "ok"          , "index"  => 23),
        array("iconName" => "telegram"    , "index"  => 24),
        array("iconName" => "vk"          , "index"  => 25),
        array("iconName" => "weibo"       , "index"  => 26),
        array("iconName" => "wechat"      , "index"  => 28),
        array("iconName" => "xing"        , "index"  => 27),
        array("iconName" =>  "phone"      ,  "index" => 28)
...
if(!function_exists('sfsi_premium_desktop_icons_order')){
    ...
    function sfsi_premium_desktop_icons_order($option5=false,$option1=false){
        ...
        // if(sfsi_premium_get_client_ip()==="115.187.55.193"){
            // var_dump($customDIcons );
        // }
        ...
        if(count($icon_name_in_order_array_to_compare)<=count(unserialize(SFSI_PLUS_ALLICONS))){
            // var_dump($icon_name_in_order_array);
            foreach(unserialize(SFSI_PLUS_ALLICONS) as $icon){
                if("facebook"===$icon){
                    $icon="fb";
                }
...
if(!function_exists('sfsi_premium_get_icons_html')){
    ...
    function sfsi_premium_get_icons_html($arrOrderIcons,$option1=false,$addLiMarkup=false,$is_front=0){
        ...
        $sfsi_premium_destop_custom_icons = @unserialize($option1['sfsi_custom_desktop_icons']);
        $sfsi_premium_mobile_custom_icons = @unserialize($option1['sfsi_custom_mobile_icons']);

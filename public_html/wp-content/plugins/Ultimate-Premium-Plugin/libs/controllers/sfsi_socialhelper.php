...
class sfsi_plus_SocialHelper
{
  ...
  public function sfsiFB_Share_Custom($permalink, $show_count = false)
  {
    ...
    $fb_share_html = "<a href='" . $shareurl . "' " . $new_window . " style='display:inline-block;'  > <img class='sfsi_premium_wicon'  data-pin-nopin='true' width='auto' height='auto' alt='fb-share-icon' title='Facebook Share' src='" . SFSI_PLUS_PLUGURL . "images/share_icons/fb_icons/" . $language . ".svg" . "'  /></a>";
    return $fb_share_html;
  }
  ...
  public function  sfsi_get_youtube_subs($user = "")
  {
    ...
    $sfsi_premium_youtube_count = unserialize(get_option('sfsi_premium_youtube_count', false));
    ...
    if ($isGlobalSharingOn || $post_id == 0) {
      ...
      if (isset($custom_pinit_img) && !empty($custom_pinit_img)) {
        ...
        if (isset($option5['sfsi_premium_featured_image_as_og_image']) && 'yes' ==  $option5['sfsi_premium_featured_image_as_og_image']) {
          ...
          $pinterest_img = $pinterest_img_obj[0];
        } else {
          $pinterest_img = "";
        }
      }
  ...
  public function sfsi_PinIt($mouse_hover_effect = '', $class = '', $url = '', $iconImg = '', $icon_opacity = 1, $style = "")
  {
    $option5 =  unserialize(get_option('sfsi_premium_section5_options', false));
    ...
    if ($option5['sfsi_premium_pinterest_sharing_texts_and_pics'] === "yes") {
      ...
      $pin_it_html    = "<a style='" . $style . "' class='sfsi_premium_pinterest_create " . $popup_target_state.$class . "'  style='cursor:pointer' onclick='sfsi_premium_pinterest_modal_images(\"" . $url . "\",\"" . urlencode(($description)) . "\")' data-description=" . urlencode(wptexturize($description)) . ">" . $iconImg . "</a>";
    } else {
      ...
      $pin_it_html = "<a data-pin-custom='true' style='cursor:pointer;" . $style . "' href='https://www.pinterest.com/pin/create/button/?url=" . urlencode($url) . "&media=" . urlencode($media) . "&description=" . ($description) . "'" . sfsi_plus_checkNewWindow($url) . " class='" . $class . "'>$iconImg</a>";
    }

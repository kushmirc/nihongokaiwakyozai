		...
		if(is_wp_error($response)){
			...
			if ( 200 !== $httpCode) {
				...
				switch ($httpCode) {				
					...
					break;
					case 403:
						$message =  __( 'For license activation your server needs to connect to the Sellcodes server ,however, it seems that your SERVER IP is blacklisted by Cloudflayer. Please contact us with this detail to resolve the issue as soon as possible.' );
					break;
				...
				if ( false === $license_data->success){
					...
					if(isset($license_data->Activation_ok) && false === $license_data->Activation_ok){
						...
						if($license_data->Activation_count == $license_data->Maximum_activations){
							...
							$message = __( 'The maximum number of activations for this license has been reached. Please acquire a new license key <a href=https://sellcodes.com/XdHlrQnc target=_blank >here</a> (use the discount code SUPPORTEDREPEATEDPURCHASE to get it 20% off). If you have any questions about it, please reach out to <a href=mailto:help@ultimatelysocial.com?subject=Question_about_additional_license target=_blank >help@ultimatelysocial.com</a>.' );
						}
					}

		...
		if(!empty($custom_img_dirPath)):
			...
			{						
				...
				if('png' == $imageExt){
					...
					$img = base64_encode(file_get_contents( $newImageUrl1 ));
...
function sfsi_plus_UploadSkins()
{
    ...
    if ( ! current_user_can( 'manage_options' ) ) {
          echo  json_encode(array("You should be admin to take this action")); exit;
			}
...
function sfsi_premium_deleteWebChatFollow()
{
	...
	$option2= (get_option('sfsi_premium_section2_options',false)) ? unserialize(get_option('sfsi_premium_section2_options',false)) : '' ;
	if(is_array($option2)){
		$option2["sfsi_premium_wechat_scan_image"]="";
		update_option('sfsi_premium_section2_options',serialize($option2));
		echo json_encode(array("res"=>"success","message"=>"removed the scan image for wechat"));
	}else{
		echo json_encode(array("res"=>"failed","message"=>"Couldn't load options"));
	}
	die;
}

...
function sfsi_plus_social_buttons_below($content, $inline = true, $sfsi_section8 = null, $gutenberg = false)
{
	...
	if (
		...
		if ($inline) {
			...
			<div style='display: " . ($inline ? "inline-block" : 'block') . ";margin-bottom: 0; margin-left: 0; margin-right: 8px; vertical-align: middle;width: auto;'>
				<span>" . $txt . "</span>
			</div>
			...
			$icons = "
			<div style='display: " . ($inline ? "inline-block" : 'block') . ";margin-bottom: 0; margin-left: 0; margin-right: 8px; margin-top: 5px; vertical-align: middle;width: auto;'>
				<span>" . $txt . "</span>
			</div>
			<div class='sfsi_plus_Sicons " . $float . "' style='" . $style . $style_parent . "'>
			";
		}
	}
	...
	if ($sfsi_section8['sfsi_plus_rectfb'] == 'yes') {
		...
		if ($sfsi_section5['sfsi_plus_Facebook_linking'] == "facebookcustomurl") {
			...
			$icons .= "<div class='sf_fb' style='display: inline-block;vertical-align: sub;width: auto;'>" . $socialObj->sfsi_plus_FBlike($userDefineLink, $show_count) . "</div>";
		} else {
			$icons .= "<div class='sf_fb' style='display: inline-block;vertical-align: sub;width: auto;'>" . $socialObj->sfsi_plus_FBlike($permalink, $show_count) . "</div>";
		}
	}
	if ($sfsi_section8['sfsi_plus_rectfbshare'] == 'yes') {
		...
		$icons .= "<div class='sf_fb' style='display: inline-block;vertical-align: middle;width: auto;'>" . $socialObj->sfsiFB_Share_Custom($permalink) . $count_html . "</div>";
	...
	if ($sfsi_section8['sfsi_plus_recttwtr'] == 'yes') {
		...
		$icons .= "<div class='sf_twiter' style='display: inline-block;vertical-align: middle;width: auto;'><a href='https://twitter.com/intent/tweet?text=" . urlencode($twitter_text) . "' " . sfsi_plus_checkNewWindow() . " style='display:inline-block' ><img nopin=nopin width='auto' class='sfsi_premium_wicon' src='" . $tweet_icon . "' alt='Tweet' title='Tweet' ></a>" . $count_html . "</div>";
	...
	if ($sfsi_section8['sfsi_plus_rectpinit'] == 'yes') {
		...
		if ($sfsi_section5['sfsi_premium_pinterest_sharing_texts_and_pics'] === "yes") {
			$icons .= "<div class='sf_pinit2' style='display: inline-block;text-align:left;margin-left:5px;margin-right:5px;vertical-align:top;'><a class ='sfsi_premium_pinterest_create' style='display:inline-block;vertical-align:bottom;'  onclick='sfsi_premium_pinterest_modal_images(\"" . $permalink . "\",\"" . $description_escaped . "\")'><img class='sfsi_premium_wicon' data-pin-nopin='true'  nopin=nopin alt='" . $alt_text . "' title='" . $alt_text . "' src='" . $pinterest_save . "'  /></a>" . $count_html . "</div>";
		} else {
			$icons .= "<div class='sf_pinit2' style='display: inline-block;text-align:left;margin-left:5px;margin-right:5px;vertical-align:top;'><a  data-pin-custom='true' style='cursor:pointer;display:inline-block;vertical-align:bottom;' href='https://pinterest.com/pin/create/button/?url=" . urlencode($permalink) . "&media=" . urlencode($media) . "&description=" . ($encoded_description) . "'" . sfsi_plus_checkNewWindow() . "><img class='sfsi_premium_wicon' nopin=nopin alt='" . $alt_text . "' title='" . $alt_text . "' src='" . $pinterest_save . "'  /></a>" . $count_html . "</div>";
		}
	...
	if ($sfsi_section8['sfsi_plus_rectlinkedin'] == 'yes') {
		...
		$icons .= "<div class='sf_linkedin' style='display: inline-block;vertical-align: top;text-align:left;width: " . $sfsiLikeWithlinkedin . "'><a href='https://www.linkedin.com/shareArticle?url=" . $permalink . "'><img class='sfsi_premium_wicon' nopin=nopin alt='Share' title='Share' src='" . $linkedin_share_icon . "'  /></a>" . $count_html . "</div>";
	...
	if ($sfsi_section8['sfsi_plus_rectreddit'] == 'yes') {
		...
		$icons .= "<div class='sf_reddit' style='display: inline-block;vertical-align: top;text-align:left;margin-top:6px;width: " . $sfsiLikeWithreddit . "'>" . sfsi_redditShareButton($permalink) . "</div>";
	}
	...
	if ((isset($option8["sfsi_plus_show_item_onposts"]) && $option8["sfsi_plus_show_item_onposts"] == "yes" && isset($option8["sfsi_plus_display_button_type"]) && $option8["sfsi_plus_display_button_type"] == "responsive_button") || $server_side) :
		...
		<?php
				...
				foreach ($sfsi_premium_responsive_icons['default_icons'] as $icon => $icon_config) {
					...
					if (false == $is_pinterest || (true == $is_pinterest && $icon_config['url'] !== "")) {
						...
						if ($icon_config['active'] == 'yes') {
							$icons .= "\t\t" . "<a " . sfsi_plus_checkNewWindow() . " href='" . ($icon_config['url'] == "" ? $share_url : do_shortcode($icon_config['url'])) . "' style='" . ($icon_config['active'] == 'yes' ? ($sfsi_premium_responsive_icons['settings']['icon_width_type'] === "Fixed icon width" ? 'display:inline-flex' : 'block') : 'display:none') . ";" . $sfsi_premium_anchor_style . "' class=" . ($sfsi_premium_responsive_icons['settings']['icon_width_type'] === "Fixed icon width" ? 'sfsi_premium_responsive_fixed_width' : 'sfsi_premium_responsive_fluid') . " >" . "\n";
						...
						if ($icon_config['active'] == 'yes') {
							...
							$icons2 .= "\t\t\t\t<img style='max-height: 25px;display:unset;margin:0' onclick='event.target.parentNode.click()' src='" . SFSI_PLUS_PLUGURL . "images/responsive-icon/" . $icon . '.svg' . "'>" . "\n";
							$icons2 .= "\t\t\t\t<span onclick='event.target.parentNode.click()' style='color:#fff' >" . ($icon_config["text"]) . "</span>" . "\n";
							$icons2 .= "\t\t\t</div>" . "\n";
							$icons .= $socialObj->sfsi_PinIt('', ($sfsi_premium_responsive_icons['settings']['icon_width_type'] === "Fixed icon width" ? 'sfsi_premium_responsive_fixed_width' : 'sfsi_premium_responsive_fluid'), $current_url, $icons2, 1, ($icon_config['active'] == 'yes' ? ($sfsi_premium_responsive_icons['settings']['icon_width_type'] === "Fixed icon width" ? 'display:inline-flex' : 'block') : 'display:none') . ";" . $sfsi_premium_anchor_style);
						}
					}
					$is_pinterest = false;
				}
				...
				foreach ($sfsi_premium_responsive_icons_custom_icons as $icon => $icon_config) {
					...
					if ($icon_config['active'] == 'yes') {
						$current_url =  $socialObj->sfsi_get_custom_share_link(strtolower($icon), $option5);
						$icons .= "\t\t" . "<a " . sfsi_plus_checkNewWindow() . " " . $sfsi_premium_open_in . " href='" . ($icon_config['url'] == "" ? "" : do_shortcode($icon_config['url'])) . "' style='" . ($icon_config['active'] == 'yes' ? 'display:inline-flex' : 'display:none') . ";" . $sfsi_premium_anchor_style . "' class=" . ($sfsi_premium_responsive_icons['settings']['icon_width_type'] === "Fixed icon width" ? 'sfsi_premium_responsive_fixed_width' : 'sfsi_premium_responsive_fluid') . "  >" . "\n";
						...
						$icons .= "\t\t\t\t<img style='max-height: 25px;margin-bottom:0 !important' src='" . (isset($icon_config['icon']) ? $icon_config['icon'] : '#') . "'>" . "\n";
						$icons .= "\t\t\t\t<span style='color:#fff' >" . ($icon_config["text"]) . "</span>" . "\n";
						$icons .= "\t\t\t</div>" . "\n";
						$icons .= "\t\t</a>" . "\n\n";
					}
		...
		function sfsi_plus_Subscribelike($permalink, $show_count, $sfsi_section5 = null)
		{
			...
			if ($sfsi_premium_section8_options['sfsi_plus_icons_DisplayCounts'] == "yes") {
				$icon = '<a href="' . $url . '" target="_blank"><img nopin=nopin src="' . $visit_icon . '" alt="onpost_follow" /></a>
		...
		function sfsi_plus_pinterest_Custom($permalink, $show_count = false)
		{
			...
			$pinit_html = "<a href='" . $pinit_html . "' style='display:inline-block;'  > <img class='sfsi_wicon'  data-pin-nopin='true' width='auto' height='auto' alt='Pin Share' title='Pin Share' src='" . SFSI_PLUS_PLUGURL . "images/share_icons/Pinterest_Save/en_US_save.svg" . "'  /></a>";
			return $pinit_html;
		}
		...
		var addthis_config = {
			...
			<a class="addthis_counter addthis_pill_style share_showhide"></a>
				...
				$atiocn .= '<div class="addthis_toolbox addthis_default_style addthis_20x20_style" addthis:url="' . $permalink . '" addthis:title="' . $post_title . '"><a class="addthis_button_compact " href="#">  <img nopin=nopin src="' . SFSI_PLUS_PLUGURL . 'images/sharebtn.png"  border="0" alt="Share" /></a></div>';
				return $atiocn;
			}
		}
		...
		function sfsi_redditShareButton($url = '')
		{
			...
			return  $ifollow = '<a href="' . $url . '" onclick="' . $onclick . '"> <img nopin=nopin src="//www.redditstatic.com/spreddit7.gif" alt="submit to reddit" style="border:0" /> </a>';
		}

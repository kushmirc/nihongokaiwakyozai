...
if (!is_plugin_active('ultimate-social-media-plus/ultimate_social_media_icons.php')) {
	...
	<?php }
		...
		<?php }
			...
			$caching_debug_option = unserialize(get_option('sfsi_premium_cache_debug_options', "a:0:{}"));
			...
			function sfsi_plus_cronstarter_activation()
			{
				...
				$sfsi_premium_cron =  unserialize(get_option('sfsi_premium_cron'));
			...
			function sfsi_premium_deserialize_options()
			{
				...
				foreach ($arr_options as $option) {
					$unserialize_option =  unserialize(get_option($option, false));
					while (is_string($unserialize_option)) {
						$unserialize_option =  unserialize($unserialize_option);
						$serialized_count++;
					}
				...
				if ($serialized_count > 0) {
					$sfsi_premium_serialize_options_list = unserialize(get_option('sfsi_premium_serialize_options', false));
			...
			add_action('init', 'sfsi_premium_update_class');
			function sfsi_premium_body_class($classes)
			{
				$classes[] = 'usm-premium-' . get_option('sfsi_premium_pluginVersion') . '-updated-' . get_option('sfsi_premium_plugin_update');
				return $classes;
			}

			function sfsi_premium_update_class()
			{
	...
	if (!defined('insPP_initialized')) new Inisev\Subs\InisevPlugPromo(__FILE__, 'sfsi-plus-options', 'USM Premium', 'usm-premium');

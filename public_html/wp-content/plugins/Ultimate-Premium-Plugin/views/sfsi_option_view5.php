...
// $option5['sfsi_premium_url_shortner_icons_names_list']   = (isset($option5['sfsi_premium_url_shortner_icons_names_list'])) ? maybe_unserialize(maybe_unserialize($option5['sfsi_premium_url_shortner_icons_names_list'])): array();
...
<div class="tab5">
	...
	<div class="row">
		...
		<div class="icons_size">
			<div class="sfsi_plus_colone">
				...
				<input name="sfsi_plus_icons_size" value="<?php echo ($option5['sfsi_plus_icons_size'] != '') ?  $option5['sfsi_plus_icons_size'] : '20'; ?>" type="text" />
				<ins>
			...
			<div class="sfsi_plus_colone">
				<div>
					...
					<!--<p>
                        <?php _e('(E.g. if you selected «1 icon per row» below, i.e. have a vertical alignment of icons, this field allows you define the space between the icon)', SFSI_PLUS_DOMAIN); ?>
                    </p> -->
				</div>
			...
			<ul class="sfsi_plus_new_alignment_option">
				...
				<li class="sfsi_plus_horiZontal_slideup">
					...
					<ins class="leave_empty">
						<span><?php _e('Leave empty if you don’t want to define this', SFSI_PLUS_DOMAIN); ?></span>
					</ins>
				</li>
				...
				<li>
					<span class="sfsi_plus_new_alignment_span">
						...
						<?php _e('Alignment of icons In the second row ', SFSI_PLUS_DOMAIN); ?>
						<p><?php _e('(with respect to icons in the first row only relevant if your icons show in two or more rows):', SFSI_PLUS_DOMAIN); ?></p>
					</span>
					<div class="field">
	...
	<div class="row sfsi_plus_url_shorting_api_type_setting">
		...
		<p><?php _e('The URL Shortener will change your shared URLs into shorter URLs. This is especially important for Twitter (due to the 140 character  limit on) but also for other sharing your content will look less clunky.', SFSI_PLUS_DOMAIN); ?>
			<br />
			...
			<div style="<?php echo $sfsi_premium_bitlyv4token_display; ?>" class="sfsi_plus_bitly_access_token">
				<label><?php _e('Your old (free) bitly access token:', SFSI_PLUS_DOMAIN); ?><br />
				</label>
				<input type="text" readonly name="sfsi_plus_url_shortner_bitly_key" value="<?php echo $option5['sfsi_plus_url_shortner_bitly_key']; ?>" style="width:40%;min-height: 40px;">
			</div>
			...
			<a style="<?php echo $sfsi_premium_bitly_display; ?>" href="https://bitly.com/oauth/authorize?client_id=<?php echo BITLY_CLIENT_ID; ?>&redirect_uri=<?php echo BITLY_REDIRECT_URL; ?>&site_url=<?php echo urlencode(site_url()); ?>&nonce=<?php echo wp_create_nonce('sfsi_premium_authorize_bitly'); ?>" onclick="sfsi_premium_open_bitly_newwindow(event)" target="_blank" class="btn btn-info sfsi_plus_bitly_authorize_button" role="button">
				...
				if ($is_bitly_new_access_token) {
					echo "<span >Reauthorize</span>";
					echo "<span style='display:none' >Move from depricated API</span>";
					echo "<span  style='display:none' >Authorize</span>";
				} elseif (isset($option5['sfsi_plus_url_shortner_bitly_key']) && !empty($option5['sfsi_plus_url_shortner_bitly_key'])) {
					echo "<span style='display:none' >Reauthorize</span>";
					echo "<span >Move from depricated API</span>";
					echo "<span  style='display:none' >Authorize</span>";
				} else {
					echo "<span style='display:none' >Reauthorize</span>";
					echo "<span style='display:none' >Move from depricated API</span>";
					echo "<span >Authorize</span>";
				}
				?>
			</a>
			<!-- ********************************* url shortner STARTS *********************************************************** -->
	...
	<div class="row">
		...
		<div class="icons_size">
			...
			if (isset($facebook_icons_lang) && !empty($facebook_icons_lang)) {
				?>
				<div class="social-img-link"><img src="<?php echo $visit_icon; ?>" alt="Facebook" /></div>
			<?php } ?>
		</div>
		...
		<div class="icons_size">
			<div class="follows-btn">
				<span><?php _e('Youtube «Visit»-icon:', SFSI_PLUS_DOMAIN); ?></span>
			</div>
			<div class="field language-field">
		...
		<div class="icons_size">
			<div class="follows-btn">
				<span><?php _e('Linkedin «Visit»-icon:', SFSI_PLUS_DOMAIN); ?></span>
			</div>
			<div class="field language-field">
		...
		<div class="icons_size">
			<span>
				<?php _e('Language for Like, Share and Subscribe buttons (Facebook, Twitter, Youtube):', SFSI_PLUS_DOMAIN); ?>
			</span>
				...
				<select name="sfsi_plus_icons_language" id="sfsi_plus_icons_language" class="language">
					...
					if (function_exists('icl_object_id') && has_filter('wpml_current_language')) :
						...
						<option value="automatic" <?php echo ($option5['sfsi_plus_icons_language'] == 'automatic') ?  'selected="selected"' : ''; ?>>
							<?php _e("Automatic (Using WPML)", SFSI_PLUS_DOMAIN); ?>
						</option>
					<?php endif;
					?>
	...
	<div class="row new_wind">
		...
		<h4>
			<?php _e('If a user clicks on your icons, where do you want to open the page?', SFSI_PLUS_DOMAIN); ?>
		</h4>
	...
	<div class="row sticking">
		...
		<p>
			<?php _e('If you decided to show your icons via a widget, you can add the effect that when the user scrolls down, the icon will stick at the  top of the screen so that they are still displayed even if the user scrolled all the way down. Do you want to do that?', SFSI_PLUS_DOMAIN); ?>
		</p>
		<div class="space">
	...
	<div class="row mouse_txt">
		...
		<div class="space">
			...
			<div class="mouseover_field sfsiplus_rss_section">
				...
				<input name="sfsi_plus_rss_MouseOverText" value="<?php echo (isset($option5['sfsi_plus_rss_MouseOverText']) && $option5['sfsi_plus_rss_MouseOverText'] != '') ?  $option5['sfsi_plus_rss_MouseOverText'] : ''; ?>" type="text" />
			</div>
			<div class="mouseover_field sfsiplus_email_section">
				<label>
					Email:
				</label>
				<input name="sfsi_plus_email_MouseOverText" value="<?php echo (isset($option5['sfsi_plus_email_MouseOverText']) && $option5['sfsi_plus_email_MouseOverText'] != '') ?  $option5['sfsi_plus_email_MouseOverText'] : ''; ?>" type="text" />
			</div>
			<div class="clear">
				<div class="mouseover_field sfsiplus_twitter_section">
					...
					<input name="sfsi_plus_twitter_MouseOverText" value="<?php echo (isset($option5['sfsi_plus_twitter_MouseOverText']) && $option5['sfsi_plus_twitter_MouseOverText'] != '') ?  $option5['sfsi_plus_twitter_MouseOverText'] : ''; ?>" type="text" />
				</div>
				<div class="mouseover_field sfsiplus_fb_section">
					<label>
						Facebook:
					</label>
					<input name="sfsi_plus_facebook_MouseOverText" value="<?php echo (isset($option5['sfsi_plus_facebook_MouseOverText']) && $option5['sfsi_plus_facebook_MouseOverText'] != '') ?  $option5['sfsi_plus_facebook_MouseOverText'] : ''; ?>" type="text" />
				</div>
			</div>
			...
			<div class="clear">
				...
				<div class="mouseover_field sfsiplus_linkedin_section">
					...
					<input name="sfsi_plus_linkedIn_MouseOverText" value="<?php echo (isset($option5['sfsi_plus_linkedIn_MouseOverText']) && $option5['sfsi_plus_linkedIn_MouseOverText'] != '') ?  $option5['sfsi_plus_linkedIn_MouseOverText'] : ''; ?>" type="text" />
				</div>
				<div class="mouseover_field sfsiplus_pinterest_section">
					<label>
						Pinterest:
					</label>
					<input name="sfsi_plus_pinterest_MouseOverText" value="<?php echo (isset($option5['sfsi_plus_pinterest_MouseOverText']) && $option5['sfsi_plus_pinterest_MouseOverText'] != '') ?  $option5['sfsi_plus_pinterest_MouseOverText'] : ''; ?>" type="text" />
				</div>
			</div>
			...
			<div class="clear">
				...
				<div class="mouseover_field sfsiplus_youtube_section">
					...
					<input name="sfsi_plus_youtube_MouseOverText" value="<?php echo (isset($option5['sfsi_plus_youtube_MouseOverText']) && $option5['sfsi_plus_youtube_MouseOverText'] != '') ?  $option5['sfsi_plus_youtube_MouseOverText'] : ''; ?>" type="text" />
				</div>
				<div class="mouseover_field sfsiplus_instagram_section">
					<label>
						Instagram:
					</label>
					<input name="sfsi_plus_instagram_MouseOverText" value="<?php echo (isset($option5['sfsi_plus_instagram_MouseOverText']) && $option5['sfsi_plus_instagram_MouseOverText'] != '') ?  $option5['sfsi_plus_instagram_MouseOverText'] : ''; ?>" type="text" />
				</div>
			</div>
			...
			<div class="clear">
				...
				<div class="mouseover_field sfsiplus_fbmessenger_section">
					...
					<input name="sfsi_plus_fbmessenger_MouseOverText" value="<?php echo isset($option5['sfsi_plus_fbmessenger_MouseOverText']) && !empty($option5['sfsi_plus_fbmessenger_MouseOverText']) ? $option5['sfsi_plus_fbmessenger_MouseOverText'] : ''; ?>" type="text" />
				</div>
				<div class="mouseover_field sfsiplus_gab_section">
					<label>
						Gab:
					</label>
					<input name="sfsi_plus_gab_MouseOverText" value="<?php echo isset($option5['sfsi_plus_gab_MouseOverText']) && !empty($option5['sfsi_plus_gab_MouseOverText']) ? $option5['sfsi_plus_gab_MouseOverText'] : ''; ?>" type="text" />
				</div>
			</div>
			...
			<div class="clear">
				...
				<div class="mouseover_field sfsiplus_mix_section">
					...
					<input name="sfsi_plus_mix_MouseOverText" value="<?php echo isset($option5['sfsi_plus_mix_MouseOverText']) && !empty($option5['sfsi_plus_mix_MouseOverText']) ? $option5['sfsi_plus_mix_MouseOverText'] : ''; ?>" type="text" />
				</div>
				...
				<div class="mouseover_field sfsiplus_ok_section">
					...
					<input name="sfsi_plus_ok_MouseOverText" value="<?php echo isset($option5['sfsi_plus_ok_MouseOverText']) && !empty($option5['sfsi_plus_ok_MouseOverText']) ? $option5['sfsi_plus_ok_MouseOverText'] : ''; ?>" type="text" />
				</div>
			</div>
			...
			<div class="clear">
				<div class="mouseover_field sfsiplus_telegram_section">
					...
					<input name="sfsi_plus_telegram_MouseOverText" value="<?php echo isset($option5['sfsi_plus_telegram_MouseOverText']) && !empty($option5['sfsi_plus_telegram_MouseOverText']) ? $option5['sfsi_plus_telegram_MouseOverText'] : ''; ?>" type="text" />
				</div>
				...
				<div class="mouseover_field sfsiplus_vk_section">
					...
					<input name="sfsi_plus_vk_MouseOverText" value="<?php echo isset($option5['sfsi_plus_vk_MouseOverText']) && !empty($option5['sfsi_plus_vk_MouseOverText']) ? $option5['sfsi_plus_vk_MouseOverText'] : ''; ?>" type="text" />
				</div>
			</div>
			...
			<div class="clear">
				<div class="mouseover_field sfsiplus_weibo_section">
					...
					<input name="sfsi_plus_weibo_MouseOverText" value="<?php echo isset($option5['sfsi_plus_weibo_MouseOverText']) && !empty($option5['sfsi_plus_weibo_MouseOverText']) ? $option5['sfsi_plus_weibo_MouseOverText'] : ''; ?>" type="text" />
				</div>
				...
				<div class="mouseover_field sfsiplus_xing_section">
					...
					<input name="sfsi_plus_xing_MouseOverText" value="<?php echo isset($option5['sfsi_plus_xing_MouseOverText']) && !empty($option5['sfsi_plus_xing_MouseOverText']) ? $option5['sfsi_plus_xing_MouseOverText'] : ''; ?>" type="text" />
				</div>
			</div>
			...
			<ul class="sfsi_plus_Facebook_linking_container">
				...
				<li>
					...
					<p class="sfsi_plus_facebooklinking_paragraph">
						<?php
						_e('A specific page should be liked (e.g. your Facebook fan page):', SFSI_PLUS_DOMAIN);
						?>
					</p>
					<div class="sfsi_plus_facebook_custom_url_field">
						<input type="text" name="sfsi_plus_facebook_linkingcustom_url" class="add" placeholder="http://" value="<?php echo (isset($option5['sfsi_plus_facebook_linkingcustom_url'])) ? $option5['sfsi_plus_facebook_linkingcustom_url'] : ''; ?>">
	...
	<div class="row">
		...
		<div class="sfsi_fb_cumulative_note">
			<p><?php _e('If you changed the URLs on your site in any way (e.g. moved to https, switched domains) then your share count will start again from 0 as the count is tied to the specific URL. To prevent that, you can have your counts from the old URLs and new URLs aggregated.', SFSI_PLUS_DOMAIN); ?>
			</p>
		</div>
		...
		<div class="sfsi_plus_cumulative_count sfsiCumulatateSection">
			...
			<ul class="sfsi_plus_cumulativecount_ul sfsi_align" style="float:none">
				...
				<li>
					...
					<label><?php _e('Off', SFSI_PLUS_DOMAIN); ?></label>
				</li>
				<li>
					<div class="sfsi_plus_cumulative_count_radio">
			...
			<div class="sfsi_premium_aggregate_options" style="display:<?php echo ($sfsi_plus_cumulative_count_active == 'no') ?  'none' : 'block' ?>">
				...
				<div>
					...
					<label style="float:none;font-weight:bold!important"><?php _e('Aggregate counts on http & https', SFSI_PLUS_DOMAIN); ?></label>
					<div class="clear:both"></div>
				</div>
				<div style="margin-left:80px;font-weight: normal!important;margin-top: -15px;color:#414951">
					<?php _e('This makes sense if you installed a SSL certificate recently (which is recommended!) and your URLs are on https rather than http.', SFSI_PLUS_DOMAIN); ?>
				</div>
				<div>
					<div style="float:left;margin-right:40px">
					...
					<label style="float:none;font-weight:bold!important"><?php _e('Aggregate counts from previous domain', SFSI_PLUS_DOMAIN); ?></label>
					<div class="clear:both"></div>

				</div>

				<div style="margin-left:80px;font-weight: normal!important;margin-top: -15px;color:#414951">
					<?php _e('Did you change your domain (e.g. from www.website.com to www.new-website.com)? Then this is the option for you.', SFSI_PLUS_DOMAIN); ?>
				</div>
				<div class="sfsi_plus_not_http_cumulative_count_active" style="height:150px;padding-top:15px;display:<?php echo ($sfsi_plus_http_cumulative_count_active == "yes") ? "none" : "block"; ?>">
					<ul class="sfsi_plus_cumulativecount_ul" style="width:100%">
						<li style="width:50%">
							<label style="float:left"><?php _e('Previous domain:', SFSI_PLUS_DOMAIN); ?></label>
							<div style="float:left">
								<input name="sfsi_plus_http_cumulative_count_previous_domain" type="text" value="<?php echo $sfsi_plus_http_cumulative_count_previous_domain; ?>" />
							</div>
							<span style="float:left;padding-left: 20px;"><?php _e('E.g. http://www.old-domain.com', SFSI_PLUS_DOMAIN) ?></span>
						</li>
						<li style="width:50%">
							<label style="float:left"><?php _e('New domain:', SFSI_PLUS_DOMAIN); ?></label>
							<div style="float:left">
								<input name="sfsi_plus_http_cumulative_count_new_domain" type="text" value="<?php echo $sfsi_plus_http_cumulative_count_new_domain; ?>" />
							</div>
							<span style="float:left;padding-left: 20px;"><?php _e('E.g. http://www.new-domain.com', SFSI_PLUS_DOMAIN) ?></span>
						</li>
				...
				<div class="sfsi_plus_custom_domain_cumulativecount_ul ">
					<label style="font-weight:bold!important"><?php _e('Aggregation will apply to:', SFSI_PLUS_DOMAIN); ?></label>
					...
					<ul class="sfsi_plus_cumulativecount_ul sfsi_align">
						...
						<li class="sfsi_align">
							...
							<label><?php _e('Facebook counts', SFSI_PLUS_DOMAIN); ?></label>
						</li>
						<li class="sfsi_align">
							<div class="sfsi_plus_cumulative_count_radio">
							...
							<label><?php _e('Pinterest counts', SFSI_PLUS_DOMAIN); ?></label>
				...
				<div style="width: fit-content;padding-top: 19px;margin-top: 18px;">
					<label style="font-weight:bold!important"><?php _e('Do you want to remove slash in URL:', SFSI_PLUS_DOMAIN); ?></label>
	...
	<div class='row'>
		<h4>
			<?php _e('Add a path to sharing URLs', SFSI_PLUS_DOMAIN); ?>
		</h4>
		<div class="icons_size">
			<ul class="sfsi_plus_new_tooltip_option" style="margin:0!important">
				<p class=""><?php _e('If you use a «Relative URL plugin» which cuts your URLs from e.g. www.your-website.com/page1 to /page1 then it may cause issues with sharing (as «/page1» isn’t a proper URL). To fix that, enter a path (e.g. www.your-website.com/) below.', SFSI_PLUS_DOMAIN); ?></p>
				<li class="sfsi_plus_new_tooltip_li">
					<div class="sfsi_plus_static_path_div sfsi_premium_d_flex_center">
						<span class="sfsi_plus_new_tooltip_span" style="width:auto;margin-top:0px">
							<?php _e('The added path before sharing URLs:', SFSI_PLUS_DOMAIN); ?></span>
						<div class="field">
							<input name="sfsi_premium_static_path" type="text" value="<?php echo isset($option5['sfsi_premium_static_path']) ? $option5['sfsi_premium_static_path'] : ''; ?>" />
						</div>
					</div>
				</li>
			</ul>
	...
	<div class="row new_wind sfsi_premium_external_js">
		<h4>
			<?php _e('Disable external JScript libraries', SFSI_PLUS_DOMAIN); ?>
		</h4>
		<p>
			<?php _e("Here you can disable our plugin from loading JavaScript libraries (which can have a big impact on your website's loading speed)", SFSI_PLUS_DOMAIN); ?>
		</p>
		<div class="sfsiplus_row_onl" <?php echo $sfsi_plus_loadjscript; ?>>
			...
			<div class="afterdisable <?php echo isset($option5['sfsi_plus_loadjscript']) && $option5['sfsi_plus_loadjscript'] == "no" ?  'show' : 'hide'  ?>">
				<p style="margin-bottom: 8px;margin-top: 8px;">
					<?php _e('Upload the JScript libraries on your server (optional)', SFSI_PLUS_DOMAIN); ?>
				</p>

				<button class="sfsi_premium_upload_btn  <?php echo !isset($option5['sfsi_plus_jscript_fileName']) || empty($option5['sfsi_plus_jscript_fileName']) ?  'show' : 'hide'  ?>" onclick="sfsi_premium_upload_javascript_libraries(event)"><?php _e('Upload ', SFSI_PLUS_DOMAIN); ?></button>
				...
				if (isset($option5['sfsi_plus_jscript_fileName']) && !empty($option5['sfsi_plus_jscript_fileName']))
					...
					<div class="sfsi_plus_other_btn  <?php echo isset($option5['sfsi_plus_jscript_fileName']) && $option5['sfsi_plus_jscript_fileName'] != "" ?  'show' : 'hide'  ?>" style="margin-bottom: 8px;">
						<div style="display:flex;align-items:center">
							...
							<button class="sfsi_premium_otherbtn" id="uploadBtn1" onclick="sfsi_premium_upload_update_javascript_libraries(event)"><?php _e('Update ', SFSI_PLUS_DOMAIN); ?></button>
							<button class="sfsi_premium_otherbtn" id="remove_project_file" onclick="sfsi_premium_delete_javascript_libraries(event)"><?php _e('Delete ', SFSI_PLUS_DOMAIN); ?></button>
							<button class="sfsi_premium_otherbtn" id="btn1" onclick="sfsi_premium_upload_more_javascript_libraries(event)"><?php _e('Add New ', SFSI_PLUS_DOMAIN); ?></button>
						</div>
					</div>
				<?php
				} else {
	...
	<div class="row new_wind">
		<h4>
			<?php _e('Hook priority', SFSI_PLUS_DOMAIN); ?>
		</h4>

		<div style="display: flex;align-items: center;">
			<p style="margin-right:10px;">
				<?php _e('Set the hook priority:', SFSI_PLUS_DOMAIN); ?>
			</p>
			<input name="sfsi_plus_hook_priority_value" type="text" value="<?php echo ($sfsi_plus_hook_priority_value != '') ?  $sfsi_plus_hook_priority_value : 20; ?>" class="add" placeholder="" />
		</div>
		<small style="color: #5a6570;">Only change the default value (20)</small>
	...
	<div class="row new_wind">
		...
		<div class="sfsiplus_row_onl">
			<p>
				<?php
				_e('Change number format', SFSI_PLUS_DOMAIN); ?>
			</p>
			<ul class="enough_waffling">
				<li>
